...
var actorChars = {
  "@": Player,
  "o": Door, 
  "=": Lava, "|": Lava, "v": Lava, "^": Lava, "<": Lava, ">": Lava, "V": Lava, "m": Lava, "n": Lava,
  "/": LGrav,
  "-": Sboost,
  "g": NGrav,
  "G": Grav,
...
Grav.prototype.type = "grav"

function Small(pos){
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.1, 0.1);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 5;
}

Small.prototype.type = "small"
...
function Lava(pos, ch) {
  ...
  if (ch == "=") {
    ...
    // Raise lava. Repeat back to this pos.
    this.speed = new Vector(0,-3);
    this.repeatPos = pos;
  } else if (ch == ">") {
    // Right lava. Repeat back to this pos.
    this.speed = new Vector(3,0);
    this.repeatPos = pos;
  } else if (ch == "<") {
    // Left lava. Repeat back to this pos.
    this.speed = new Vector(-3,0);
    this.repeatPos = pos;
  } else if (ch == "V") {
    // Fast drip lava. Repeat back to this pos.
    this.speed = new Vector(0,10);
    this.repeatPos = pos;
  } else if (ch == "m") {
    // Fast Right lava. Repeat back to this pos.
    this.speed = new Vector(21,0);
    this.repeatPos = pos;
  } else if (ch == "n") {
    // Fast Left lava. Repeat back to this pos.
    this.speed = new Vector(-21,0);
    this.repeatPos = pos;
  }
}
...
Player.prototype.act = function(step, level, keys) {
  ...
  LGrav.prototype.act = function(step, level, keys) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist * 5;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
  };

  Sboost.prototype.act = function(step, level, keys) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist * 5;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
  };

  NGrav.prototype.act = function(step, level, keys) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist * 5;
    this.pos = this.basePos.plus(new Vector(wobblePos, 0));
  };

  Small.prototype.act = function(step, level, keys) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist * 1;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
  };

  Grav.prototype.act = function(step, level, keys) {
    this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist * 5;
    this.pos = this.basePos.plus(new Vector(wobblePos, 0));
  };
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    gravity = 30;
    playerXSpeed = 7;
    this.player.size = new Vector (0.8,1.5);
    this.finishDelay = 1;
  } else if (type == "door") {
    ...
    this.actors = this.actors.filter(function(other) {
      ...
      this.player.size = new Vector (0.8,0.75);
      this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
  }
